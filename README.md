...
面向 **泰国 / 澳洲**（及更多市场）的 **GAC 广汽国际**（AION/昊铂 Hyper 为子品牌）新能源车主与潜客的 **RAG 智能客服**：意图路由 + 状态机做引导转化，向量语义检索 + LLM 生成回答，知识内容按市场隔离、多语言原生。
...
- **语言跟随**：回复按**下拉框所选语言**（`lang_hint`）生成（中/英/泰/西原生，其它语言 LLM 兜底），不写死语言码、不看消息语言。品牌为 **GAC（广汽国际）**，AION/昊铂 Hyper 为子品牌。
...
| 品牌 | **GAC（广汽国际）**为母品牌，AION/昊铂 Hyper 为子品牌；界面/问候/提示词全部 GAC 化 |
